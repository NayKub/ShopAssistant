...
function App() {
    ...
    const renderView = () => {
        switch (currentView) {
            ...
            case VIEWS.SETTINGS:
                return <SettingsPage navigateTo={navigateTo} onClose={() => setCurrentView(VIEWS.SALES)} />;
            default:
    ...
    return (
        <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-[#121212]' : 'bg-white'}`}>
            {renderView()}
        </div>
    );
}
